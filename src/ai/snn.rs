...
use crate::ethics::EthicsFilter; // Import bộ lọc
...
impl SNNCore {
    ...
    // Xử lý văn bản có kiểm duyệt
    pub async fn process_text(&self, text: &str) -> (f32, String, String) {
        // 1. KIỂM TRA ĐẠO ĐỨC (MỚI)
        if let Err(e) = EthicsFilter::check(text) {
            println!("{}", e);
            return (0.0, "⛔ BLOCKED".into(), e);
        }

        // 2. Xử lý Logic AI (như cũ)
        ...
        if let Ok(res) = self.oracle.smart_search(text).await { if !res.contains("Không tìm thấy") { ans = res; } }
        ...
        } else { ans = "Không tìm thấy dữ liệu phù hợp.".into(); }
